...
async function initIndex(){const[matches,team,one,weapons]=await Promise.all([loadJson(DATA_FILES.matches,[]),loadJson(DATA_FILES.teamElo,[]),loadJson(DATA_FILES.oneElo,[]),loadJson(DATA_FILES.weaponStats,{})]);document.getElementById("teamEloBody").innerHTML=rankRows(team,10);document.getElementById("oneEloBody").innerHTML=rankRows(one,10);renderWeapons(weapons);renderMatches(matches,"matchList",12);const names=new Set();(matches||[]).forEach(m=>getPlayers(m).forEach(p=>names.add(pname(p))));document.getElementById("totalMatches").textContent=fmt(matches.length);document.getElementById("totalPlayers").textContent=fmt(names.size||new Set([...team,...one].map(x=>x.player||x.name)).size);const lm=matches[matches.length-1];document.getElementById("latestMatch").textContent=lm?(lm.map||lm.mapname||"Match"):"—";const top=(team||[]).slice().sort((a,b)=>num(b.elo)-num(a.elo))[0];document.getElementById("topTeam").textContent=top?`${top.player||top.name} · ${fmt(top.elo)}`:"—";document.getElementById("dataStatus").textContent=matches.length||team.length?"Data connected":"Waiting for JSON files"}
...
async function initPlayer(){const name=new URLSearchParams(location.search).get("name")||"Unknown";document.title=`${name} · Q2Stats`;document.getElementById("playerName").textContent=name;const[matches,team,one,hist]=await Promise.all([loadJson(DATA_FILES.matches,[]),loadJson(DATA_FILES.teamElo,[]),loadJson(DATA_FILES.oneElo,[]),loadJson(DATA_FILES.eloHistory,[])]);const a=aggregatePlayer(matches,name),tr=(team||[]).find(x=>(x.player||x.name)===name),or=(one||[]).find(x=>(x.player||x.name)===name),wr=a.games?a.wins/a.games*100:(tr?.winrate||0),kd=a.deaths?a.kills/a.deaths:a.kills,avg=a.games?a.dealt/a.games:0;document.getElementById("heroElo").textContent=tr?fmt(tr.elo):"—";document.getElementById("pTeamElo").textContent=tr?fmt(tr.elo):"—";document.getElementById("p1v1Elo").textContent=or?fmt(or.elo):"—";document.getElementById("pMatches").textContent=fmt(a.games||tr?.matches||0);document.getElementById("pWinrate").textContent=pct(wr);document.getElementById("pKD").textContent=kd.toFixed(2);document.getElementById("pAvgDamage").textContent=fmt(avg);document.getElementById("playerSub").textContent=`${a.games} recorded matches · ${fmt(a.kills)} kills · ${fmt(a.dealt)} total dealt damage`;const combat=[["Kills",a.kills],["Deaths",a.deaths],["Damage dealt",a.dealt],["Damage received",a.received],["Suicides",a.suicides],["Teamkills",a.teamkills]];document.getElementById("combatStats").innerHTML=combat.map(([k,v])=>`<div class="stat-item"><span>${k}</span><strong>${fmt(v)}</strong></div>`).join("");const ws=Object.entries(a.weapons).map(([w,x])=>({w,acc:x.accN?x.accSum/x.accN:0,kills:x.kills,matches:x.matches})).sort((x,y)=>y.kills-x.kills);document.getElementById("weaponBody").innerHTML=ws.length?ws.map(x=>`<tr><td>${esc(x.w)}</td><td>${pct(x.acc)}</td><td>${fmt(x.kills)}</td><td>${fmt(x.matches)}</td></tr>`).join(""):'<tr><td colspan="4">No weapon data</td></tr>';const playerMatches=matches.map((m,id)=>({__match:m,__id:id})).filter(x=>playerResult(x.__match,name));renderMatches(playerMatches,"playerMatches",10);
